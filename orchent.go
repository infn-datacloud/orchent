...
var (
	...
	lsDepExcludedState = lsDep.Flag("excludedstate", "filter the deployments, the dployments in given state(s) are excluded from list").Short('x').String()
...
func deployments_list(base *sling.Sling, user string, group string, excludedstate string, before string, after string) {
	path := "./deployments"
	query_params := []string{}
	if user != "" {
	...
	if excludedstate != "" {
		query_params = append(query_params, "excludedState="+excludedstate)
	}
...
func main() {
	...
	case lsDep.FullCommand():
		deployments_list(base, *lsDepUser, *lsDepGroup, *lsDepExcludedState, *lsDepBefore, *lsDepAfter)
